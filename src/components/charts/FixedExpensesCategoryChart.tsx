...
// Custom tooltip for Fixed Expenses pie chart
const FixedExpensesPieTooltip = ({ active, payload }: any) => {
...
interface FixedExpensesCategoryChartProps {
  data: any[];
  total: number;
}

export default function FixedExpensesCategoryChart({ data, total }: FixedExpensesCategoryChartProps) {
      ...
      <PieChart>
        <defs>
          {chartData.map((entry, index) => (
            <linearGradient key={index} id={`fixed-gradient-${index}`} x1="0" y1="0" x2="1" y2="1">
        ...
        <Tooltip content={<FixedExpensesPieTooltip />} />
      </PieChart>
    </ResponsiveContainer>
  );
}
